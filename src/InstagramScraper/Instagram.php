...
class Instagram
{
    ...
    public function getMediaCommentsByCode($code, $count = 10, $maxId = null, $paginateInd = false)
    {
        ...
        while ($hasPrevious && $index < $count) {
            ...
            $commentNode = Endpoints::getCommentNode($jsonResponse['data']['shortcode_media']);


            if (
                !isset($jsonResponse['data']['shortcode_media'][$commentNode]['edges'])
                || !isset($jsonResponse['data']['shortcode_media'][$commentNode]['count'])
                || !isset($jsonResponse['data']['shortcode_media'][$commentNode]['page_info']['has_next_page'])
                || !array_key_exists('end_cursor', $jsonResponse['data']['shortcode_media'][$commentNode]['page_info'])
            ) {
                throw new InstagramException('Response code is ' . $response->code . '. Body: ' . static::getErrorBody($response->body) . ' Something went wrong. Please report issue.', $response->code);
            }

            $nodes = $jsonResponse['data']['shortcode_media'][$commentNode]['edges'];
            $hasPrevious = $jsonResponse['data']['shortcode_media'][$commentNode]['page_info']['has_next_page'];
            $numberOfComments = $jsonResponse['data']['shortcode_media'][$commentNode]['count'];
            $maxId = $jsonResponse['data']['shortcode_media'][$commentNode]['page_info']['end_cursor'];
